...
import numpy as np
import re 
from scipy.stats import t # <--- TAMBAHAN: Untuk menghitung r-tabel
...
# --- Pastikan ada file style.css di folder yang sama, atau hapus bagian ini jika tidak ada. ---
...
# --- Fungsi Bantuan untuk r-tabel (TAMBAHAN PENTING) ---
def calculate_r_table(N, alpha=0.05):
    """Menghitung nilai r-tabel berdasarkan t-student dengan df = N-2"""
    if N <= 2:
        return np.nan
    # Dua sisi (two-tailed)
    df = N - 2
    # Mencari nilai t untuk alpha/2 (0.025)
    t_val = t.ppf(1 - alpha/2, df) 
    # Rumus r-tabel: r = t / sqrt(t^2 + df)
    r_table = t_val / np.sqrt(t_val**2 + df)
    return r_table
# --- Akhir Fungsi r-tabel ---
...
# --- ### HALAMAN KUESIONER (Tidak Berubah) ### ---
...
# --- ### HALAMAN ADMIN (PERBAIKAN SINTAKS UJI INSTRUMEN & HIPOTESIS) ### ---
    ...
    try:
        ...
        # Hitung r-tabel berdasarkan jumlah responden
        # Perhatikan: r-tabel hanya bisa dihitung jika N > 2 (df > 0)
        R_TABLE_005 = calculate_r_table(num_responden)
        ...
        with tab2:
            ...
            if analysis_allowed:
                ...
                # --- PERBAIKAN UJI VALIDITAS: Menambahkan R-Tabel dan Keterangan ---
                if np.isnan(R_TABLE_005):
                    st.warning("Jumlah responden tidak cukup untuk menghitung nilai r-tabel (N harus > 2).")
                    ...
                    for indicator, qs in questions.items():
                        ...
                        if len(cols_exist) > 1:
                            ...
                            for item_col in cols_exist:
                                ...
                                # Gunakan R_TABLE_FOR_COMPARE untuk penentuan status
                ...
                # --- PERBAIKAN UJI RELIABILITAS: Menambahkan R-Tabel Cronbach's Alpha ---
                R_TABLE_CRONBACH = 0.7
                st.write(f"Variabel dianggap **reliabel** jika nilai Cronbach's Alpha **>= {R_TABLE_CRONBACH}**.")
                ...
                if num_responden > 1:
                    for indicator, qs in questions.items():
                        ...
                        if len(cols_exist) > 1:
                            ...
                            # Membandingkan dengan 0.7
                            status = "Reliabel" if alpha[0] >= R_TABLE_CRONBACH else "Kurang Reliabel"
                            reliability_results.append({
                                "Variabel": indicator, 
                                "Cronbach's Alpha": f"{alpha[0]:.3f}",
                                "r-tabel (Min. Alpha)": f"{R_TABLE_CRONBACH:.1f}", # <--- Kolom r-tabel (Min. Alpha)
                                "Keterangan": status
                            })
        ...
        with tab3:
            ...
            st.write("Hipotesis **terdukung (signifikan)** jika `p-value` **$\le 0.05$**.")

            if analysis_allowed:
                try:
                    # Model SEM (Tidak berubah)
                    model_desc = """
                    # Measurement Model
                    PU =~ PU_1 + PU_2 + PU_3
                    ...
                    structural_estimates = structural_estimates[['lval', 'rval', 'Estimate', 'Std. Err', 'z-value', 'p-value']].copy()
